...
public:
    ...
    inline void setValue(int val)
    {
		...
		q->setValue(val);
		emit q->doubleValueChanged(val * q->discrete_step);
    }
...
*/
QtThumbWheel::QtThumbWheel(double minimum, double maximum, double pageStep,
                           double value, Qt::Orientation orientation,
                           QWidget *parent)
    : QAbstractSlider(parent), d_ptr(new QtThumbWheelPrivate(this))
{
	setRange(minimum / discrete_step, maximum / discrete_step);
    setValue(value / discrete_step);
    setSingleStep(1);
...
void QtThumbWheel::setDoubleValue(double value) {
	Q_D(QtThumbWheel);
	d->setValue((int)(value / discrete_step));
}
